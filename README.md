...
Users can also connect non-custodial wallets to Vencura if they wish to transact without revealing their identity. Non-custodial wallets will not have their balances or transactions tracked internally in Vencura's database. For these wallets, Vencura serves only as an interface that users can use to transact on the Ethereum Sepolia network.
